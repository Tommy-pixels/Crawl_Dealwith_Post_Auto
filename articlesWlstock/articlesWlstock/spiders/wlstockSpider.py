...
class WlstockSpider(scrapy.Spider):
    name = 'wlstockSpider'
    start_urls = 'https://www.wlstock.com/WLCombat/GetData.aspx'
    ...
    def parse_articleList(self, response):
        articleinfo_lis = json.loads(response.text)['list']
        url_list =[]
        for article in articleinfo_lis:
            publish_time = article.split("'tar'>")[-1].split('</p>')[0].split(' ')[0].replace('月','').replace('日','')
            url = article.split("<h3>")[-1].split('</h3>')[0].split("href='")[-1].split("'")[0]
            title = article.split("<h3>")[-1].split('</h3>')[0].split(" >")[-1].split("<")[0]
            if(publish_time == Contraler_Time.getCurDate('%m%d')):
