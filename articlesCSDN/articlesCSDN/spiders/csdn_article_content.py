...
class CSDNSpider(scrapy.Spider):
    ...
    def start_requests(self):
        sql_origin = 'SELECT `id`,`ori_url`,`title` FROM `dbfreeh`.`tb_article` WHERE (`id`>8000 and `id`<9000) and (`note` is null or `note`="");'
    ...
    def parse_content(self, response, id_a):
        ...
        for p in pList:
            ...
            lis_continue = ['本分享为', 'QQ交流群', '更多分享', '作者：', '来源：', '原文：', '版权声明：', '文章出自', '公众号：', '抖音号：', '版权声明：','原文链接：','重金招聘',
                            ...
                            '|知乎','|博客','交流群','转发吧','点“在看”','文末福利', '免费获取','文/', 'CSDN编者','作者介绍','头|','CSDN：','扫描上方二维码','点击','作者｜',
                            '校对｜','微信群','关注']
            check = False
